...
class InferenceEngine(AbstractInferenceEngine):
    ...
    def __init__(self, model_path):
        ...
        self.lib = CDLL("/main/darknet_v4/libdarknetv4.so", RTLD_GLOBAL)
        ...
        self.meta_path = os.path.join(model_path, 'obj.data')
        self.meta = self.load_meta(self.meta_path.encode('utf-8'))
    ...
    def load(self):
        with open(self.__ensure__ctype__string(os.path.join(self.model_path, 'config.json'))) as f:
            data = json.load(f)
        try:
    ...
    async def infer(self, input_data, draw, predict_batch: bool = False):
        ...
        dets = self.get_network_boxes(self.net, width, height, detection_threshold, hier_threshold, self.pmap,
                                      self.relative,
                                      pnum, self.letter)
        num = pnum[0]
